...
# ============== RIDES / CLIENTES (Emisora Uber-like) ==============
clients_collection = db['clients']  # Cliente accounts (phone + first_name + last_name)
otp_codes_collection = db['otp_codes']  # OTP verification codes for client login
rides_collection = db['rides']  # Ride requests (ASAP / scheduled)
driver_qrs_collection = db['driver_qrs']  # QR tokens generated by drivers to onboard clients
password_reset_tokens_collection = db['password_reset_tokens']  # Password reset tokens (drivers + clients)
client_addresses_collection = db['client_addresses']  # Frequent origin/destination addresses per client
ride_ratings_collection = db['ride_ratings']  # Star ratings between clients and drivers (post-ride)
ride_blocks_collection = db['ride_blocks']  # Two-way mute lists (client blocks driver / driver blocks client)
