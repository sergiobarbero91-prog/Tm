...
# Configure logging - use JSON format in production for better log analysis
import sys
...
# Use JSON logging if LOG_FORMAT=json is set, otherwise use standard format
...
class FlightArrival(BaseModel):
    ...
    status: Optional[str] = None
    delay_minutes: Optional[int] = None  # Minutos de retraso
    belt: Optional[str] = None  # Cinta de equipaje
    aircraft: Optional[str] = None  # Tipo de aeronave
    is_large: Optional[bool] = False  # Wide-body (>250 pax)

class TerminalData(BaseModel):
    terminal: str
    arrivals: List[FlightArrival]
    total_next_30min: int  # Próximos (En tierra, En vuelo, En hora, Retrasado)
    total_next_60min: int
    is_winner_30min: bool = False
    is_winner_60min: bool = False
    delivering_30min: Optional[int] = 0  # Entregando equipaje
    delivering_60min: Optional[int] = 0
    # Weighted score fields
    score_30min: Optional[float] = None  # Weighted score for 30min window
    score_60min: Optional[float] = None  # Weighted score for 60min window
    past_30min: Optional[int] = None     # Finalizados
    past_60min: Optional[int] = None
    # Next half hour AFTER the selected window (early-warning lookahead)
    next_after_30min: Optional[int] = 0  # Flights arriving in [+30, +60] from now
    next_after_60min: Optional[int] = 0  # Flights arriving in [+60, +90] from now
    # Wide-body / Large aircraft counters
    large_30min: Optional[int] = 0       # Wide-body flights in 30min window
    large_60min: Optional[int] = 0       # Wide-body flights in 60min window
    # Saturation / demand prediction (0-100)
    saturation_30min: Optional[int] = 0  # Predicted demand level 0-100 for 30min window
    saturation_60min: Optional[int] = 0  # Predicted demand level 0-100 for 60min window
    saturation_level_30min: Optional[str] = "baja"  # baja / media / alta
    saturation_level_60min: Optional[str] = "baja"
    # Instant demand (taxi pressure RIGHT NOW, last 30 min only)
    instant_demand_pct: Optional[float] = 0.0     # Can exceed 100
    instant_demand_level: Optional[str] = "green"  # green / yellow / red / critical
    instant_demand_points: Optional[float] = 0.0   # Raw points (debug/aggregation)
...
# Keep HEADERS for backward compatibility but use get_headers() for new requests
HEADERS = get_headers()
...
async def fetch_adif_arrivals_scrape(station_id: str) -> List[Dict]:
    """Scrape train arrivals from ADIF website HTML.
    
    For Atocha (60000): Gets AV/Media/Larga distancia trains.
    For Chamartín (17000): Gets Cercanías trains (AV requires JS filter).
    
    New ADIF HTML format (2025+):
    - Table with headers: Hora de llegada, Origen, Línea, Vía
    - Delayed trains: time cell contains two times concatenated e.g. "22:1022:46"
    - Train line format: "RF - AVE03190" or "C4a" (Cercanías)
    """
    arrivals = []
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=HEADERS, timeout=30) as response:
                if response.status != 200:
                    logger.warning(f"ADIF returned {response.status} for {station_id}")
                    return []
                
                html = await response.text()
                soup = BeautifulSoup(html, 'lxml')
                
                # Find arrivals table (the one with "Hora de llegada" header)
                arrival_table = None
                for table in soup.find_all('table'):
                    headers = [th.get_text(strip=True) for th in table.find_all('th')]
                    if any('llegada' in h.lower() for h in headers):
                        arrival_table = table
                        break
                
                if not arrival_table:
                    logger.warning(f"Station {station_id}: No arrival table found")
                    return []
                
                rows = arrival_table.find_all('tr')[1:]  # Skip header row
                ...
                for row in rows:
                    try:
                        ...
                        # Column 0: Time (may contain "X min" for Cercanías or "HH:MM" or "HH:MMHH:MM" for delayed)
                        ...
                        # Column 2: Train line (e.g., "RF - AVE03190", "C4a", "IL - IRYO06200")
                        train_info = cols[2].get_text(strip=True)
                        
                        # Column 3: Platform/Vía
                        platform = cols[3].get_text(strip=True) if len(cols) > 3 else "-"
                        
                        # Parse time - handle delays
                        scheduled_time = None
                        real_time = None
                        delay_minutes = None
                        status = "En hora"
                        ...
                        if 'min' in time_text:
                            # Cercanías format: "X min" - calculate absolute time
                            try:
                            ...
                            time_match = re.findall(r'(\d{1,2}:\d{2})', time_text)
                            if len(time_match) >= 2:
                                scheduled_time = normalize_time_string(time_match[0])
                                real_time = normalize_time_string(time_match[1])
                                # Calculate delay
                                try:
                                    s_parts = scheduled_time.split(":")
                                    r_parts = real_time.split(":")
                                    s_mins = int(s_parts[0]) * 60 + int(s_parts[1])
                                    r_mins = int(r_parts[0]) * 60 + int(r_parts[1])
                                    delay = r_mins - s_mins
                                    if delay < -120:
                                        delay += 24 * 60
                                    delay_minutes = delay
                        ...
                        # AV format: "RF - AVE03190" or "IL - IRYO06200"
                        av_match = re.search(r'(?:[A-Z]{2}\s*-\s*)?([A-Z]+)(\d+)', train_info)
                        if av_match:
                            train_type = av_match.group(1)
                            train_number = av_match.group(2)
                        else:
                            # Cercanías format: "C4a", "C10", "C1"
                            cerc_match = re.match(r'(C\d+[a-z]?)', train_info)
                            if cerc_match:
                                train_type = cerc_match.group(1)
                                train_number = ""
                            else:
                                train_type = train_info[:6] if train_info else "TREN"
                                train_number = ""
                        
                        # For Atocha (60000): only include media/larga distancia
                        # For Chamartín (17000): include everything (Cercanías is what's available)
                        if station_id == "60000":
                        ...
                        arrivals.append({
                            ...
                            "train_type": train_type.upper(),
                            "train_number": train_number,
                            "platform": platform if platform and platform != "" else "-",
                            "status": status,
                            "delay_minutes": delay_minutes,
                            "source": "ADIF"
                        })
                    except Exception as e:
                        logger.debug(f"Error parsing ADIF row: {e}")
                        continue
    except Exception as e:
        logger.error(f"Error scraping ADIF HTML for station {station_id}: {e}")
    
    if arrivals:
        logger.info(f"Station {station_id}: Scraped {len(arrivals)} trains from ADIF HTML")
    else:
...
# Wide-body / "Grande" aircraft codes (>250 pax typical) - module level for reuse
...
def is_large_aircraft(aircraft_code: Optional[str]) -> bool:
    return bool(aircraft_code and aircraft_code.upper() in LARGE_AIRCRAFT_CODES)


def calculate_saturation(p: int, e: int, s: int, large: int, f: int = 0) -> tuple:
    """Calculate demand saturation 0-100 from counters.
    
    Calibrated against real Barajas traffic:
    - ALTA (~70-90%): rush hour with ~30 próximos + 7 equipaje
    - MEDIA: moderate activity
    - BAJA: 1-5 flights with 1-2 in IBK = quiet
    
    Weights:
    - P (próximos):  1.0  - flights coming soon
    - E (equipaje):  2.0  - passengers leaving NOW (highest)
    - F (finalizado): 0.3 - flights that already left, passengers mostly gone
    - S (siguientes): 0.5 - lookahead
    - GRANDE: 1.5 boost - wide-body ≈ 3x more passengers
    
    Scale: 50 points = 100% saturation (~30 P + 7 E + few extras = saturated)
    
    Returns (saturation_0_100, level_str)
    Level: baja (<25), media (25-60), alta (>=60)
    """
    points = (p * 1.0) + (e * 2.0) + (f * 0.3) + (s * 0.5) + (large * 1.5)
    # 50 points = 100% saturation
    saturation = min(100, int(round(points * 2)))
    if saturation >= 60:
        level = "alta"
    elif saturation >= 25:
        level = "media"
...
def calculate_instant_demand(arrivals: List[Dict]) -> dict:
    """Compute the **instant** taxi-demand pressure for a terminal.

    Spec (Demanda en este Momento, last 30 min window only):
      - FINALIZADO  0-15 min ago:    +1.0
      - FINALIZADO  16-30 min ago:   +0.3
      - ENTREGANDO EQUIPAJE >15 min: +0.8  (passengers still waiting baggage)
      - ENTREGANDO EQUIPAJE <15 min: +0.4  (just landed, baggage not out yet)
      - EN TIERRA:                   +0.2  (taxiing, gate)
      - Multiplier x1.5 for large/wide-body aircraft
      - Saturation %: total_points * 10  (so 10 weighted flights = 100%, can exceed 100%)

    Returns dict with pct (float, can be >100), level (green/yellow/red/critical),
    and points (raw score) so the frontend can compute deltas/trends.
    """
    now = datetime.now(MADRID_TZ)
    points = 0.0

    for arr in arrivals:
        status = (arr.get("status") or "").strip()
        if status not in ("Finalizado", "Entregando equipaje", "En tierra"):
            continue

        # Determine how long ago the flight landed
        minutes_ago: Optional[float] = None
        time_str = arr.get("time", "")
        if time_str:
            try:
                hh, mm = time_str.split(":")
                arr_time = now.replace(hour=int(hh), minute=int(mm),
                                       second=0, microsecond=0)
                if arr_time > now + timedelta(hours=2):
                    arr_time -= timedelta(days=1)
                minutes_ago = (now - arr_time).total_seconds() / 60.0
            except Exception:
                minutes_ago = None
        ...
        if status == "Finalizado":
            if minutes_ago is None or minutes_ago < 0:
                base = 0.0  # already gone or unknown
            elif minutes_ago <= 15:
                base = 1.0
            elif minutes_ago <= 30:
                base = 0.3
                ...
                base = 0.0
        elif status == "Entregando equipaje":
            # Default to >15 (older arrivals are usually still in IBK when "old")
            if minutes_ago is not None and minutes_ago < 15:
                base = 0.4
            else:
                base = 0.8
        elif status == "En tierra":
            base = 0.2

        if base == 0.0:
            continue

        # Large-aircraft multiplier
        if is_large_aircraft(arr.get("aircraft")):
            base *= 1.5

        points += base

    pct = round(points * 10.0, 1)  # can exceed 100
    ...
    return {"pct": pct, "level": level, "points": round(points, 2)}


async def fetch_aena_arrivals() -> Dict[str, List[Dict]]:
    """Fetch real flight arrivals from AENA official API (Infovuelos)."""
    terminal_arrivals = {t: [] for t in TERMINALS}
    
    now = datetime.now(MADRID_TZ)
    
    # AENA status code mapping
    AENA_STATUS_MAP = {
        "FLY": "En vuelo",
        "LND": "En tierra",
        "IBK": "Entregando equipaje",
        "OPE": "En tierra",
        "FNL": "Finalizado",
        "RET": "Retrasado",
        "HOR": "En hora",
    ...
    url = "https://www.aena.es/sites/Satellite?pagename=AENA_ConsultarVuelos&airport=MAD&flightType=L&limit=500&dosDias=si"
            ...
            async with session.get(url, headers=headers, timeout=aiohttp.ClientTimeout(total=30)) as response:
                if response.status == 200:
                    ...
                    for flight in flights:
                        try:
                            # Get terminal and normalize
                            terminal = flight.get("terminal", "").strip()
                            terminal = terminal.replace("T4-S", "T4S").replace("T4S ", "T4S")
                            if terminal not in terminal_arrivals:
                                continue
                            
                            # Get status
                            status_code = flight.get("estado", "")
                            status = AENA_STATUS_MAP.get(status_code, "En hora")
                            
                            # Skip cancelled and diverted
                            if status in ["Cancelado", "Desviado"]:
                                continue
                            
                            # For "Finalizado" - check if it's been more than 60 minutes
                            if status == "Finalizado":
                                try:
                                    est_time = flight.get("horaEstimada", "")
                                    if est_time:
                                        parts = est_time.split(":")
                                        est_mins = int(parts[0]) * 60 + int(parts[1])
                                        now_mins = now.hour * 60 + now.minute
                                        mins_since = now_mins - est_mins
                                        if mins_since < 0:
                                            mins_since += 24 * 60
                                        if mins_since > 60:
                                            continue  # Skip flights finalized more than 60 min ago
                                except (ValueError, TypeError):
                                    pass
                            
                            # Parse times
                            scheduled_raw = flight.get("horaProgramada", "")
                            estimated_raw = flight.get("horaEstimada", "")
                            
                            scheduled_time = scheduled_raw[:5] if scheduled_raw else ""
                            estimated_time = estimated_raw[:5] if estimated_raw else scheduled_time
                            
                            # Use estimated time as the display time
                            final_time = estimated_time or scheduled_time
                            if not final_time:
                                continue
                            
                            # Get origin city - clean up long names
                            origin_iata = flight.get("iataOtro", "")
                            origin = flight.get("ciudadIataOtro", "")
                            if origin:
                                origin = origin.title()
                                # Clean up long AENA city names
                                origin = re.sub(r'\s*/\s*\w+', '', origin)  # Remove "/ Fiumicino" etc
                                origin = re.sub(r'-El Prat.*$', '', origin)  # "Barcelona-El Prat..." -> "Barcelona"
                                origin = re.sub(r'-Elche.*$', '', origin)  # "Alicante-Elche..." -> "Alicante"
                                origin = re.sub(r'\s+Raisi$', '', origin)  # "Palermo Raisi" -> "Palermo"
                                origin = re.sub(r'\s+Galilei$', '', origin)  # "Pisa Galilei" -> "Pisa"
                                origin = re.sub(r'\s*\(.*?\)', '', origin)  # Remove parenthetical
                                origin = origin.strip()
                            else:
                                origin = origin_iata or "Unknown"
                            
                            # Deduplicate codeshares: group by physical flight
                            # Use (scheduled_time, origin_iata, terminal) as the physical flight key
                            physical_key = f"{scheduled_time}_{origin_iata}_{terminal}"
                            if physical_key in seen_flights:
                                continue  # Skip codeshare duplicates
                            seen_flights.add(physical_key)
                            
                            # Get airline info - use the operating carrier
                            airline = flight.get("nombreCompania", flight.get("compania", "Unknown"))
                            iata_comp = flight.get("iataCompania", "")
                            num_vuelo = flight.get("numVuelo", "")
                            flight_number = f"{iata_comp}{num_vuelo}"
                            
                            # Calculate delay
                            delay_minutes = None
                            if scheduled_time and estimated_time and scheduled_time != estimated_time:
                                try:
                                    s_parts = scheduled_time.split(":")
                                    e_parts = estimated_time.split(":")
                                    s_mins = int(s_parts[0]) * 60 + int(e_parts[1])
                                    e_mins = int(e_parts[0]) * 60 + int(e_parts[1])
                                    delay = e_mins - s_mins
                                    if delay < -120:
                                        delay += 24 * 60
                                    delay_minutes = delay
                            ...
                            # Extra info from AENA
                            belt_raw = flight.get("cintaPrimera")
                            hall_raw = flight.get("salaPrimera")
                            aircraft_raw = flight.get("tipoAeronave")
                            belt = str(belt_raw) if belt_raw and str(belt_raw) != "null" else None
                            hall = str(hall_raw) if hall_raw and str(hall_raw) != "null" else None
                            aircraft = str(aircraft_raw) if aircraft_raw and str(aircraft_raw) != "null" else None
                            ...
                            terminal_arrivals[terminal].append({
                                ...
                                "airline": airline,
                                "terminal": terminal,
                                "gate": hall if hall else "-",
                                "status": status,
                                "delay_minutes": delay_minutes,
                                "belt": belt,
                                "aircraft": aircraft,
                                "is_large": is_large_aircraft(aircraft),
                            })
                        except Exception as e:
                            logger.debug(f"Error parsing AENA flight: {e}")
                            continue
                else:
                    logger.error(f"AENA API returned status {response.status}")
    except Exception as e:
        logger.error(f"Error fetching AENA API: {e}")
        # Fallback to old scraping method
        return await fetch_aena_arrivals_fallback()
    
    # Sort arrivals by relevance: landed first, then by proximity to now
    landed_statuses = {"En tierra", "Entregando equipaje", "Finalizado"}
    now_mins = now.hour * 60 + now.minute
    
    def flight_sort_key(flight):
        status = flight.get("status", "")
        try:
            t_parts = flight["time"].split(":")
            f_mins = int(t_parts[0]) * 60 + int(t_parts[1])
        ...
        if status in landed_statuses:
            return (0, diff)  # Landed flights first, sorted by recency
        elif -60 <= diff <= 120:
            return (1, diff)  # Flights within -1h to +2h next
        else:
            return (2, diff)  # Far future flights last
    
    for terminal in TERMINALS:
        terminal_arrivals[terminal].sort(key=flight_sort_key)
    
    total = sum(len(v) for v in terminal_arrivals.values())
    logger.info(f"Flights fetched: {total} total across all terminals (AENA API)")
    
    # Track each flight to detect Finalizado transitions (AENA drops finalized flights)
    if total > 0:
        try:
            await upsert_tracked_flights(terminal_arrivals)
        except Exception as e:
            logger.error(f"[FlightTrack] Error upserting tracked flights: {e}")
    
    return terminal_arrivals


async def fetch_aena_arrivals_fallback() -> Dict[str, List[Dict]]:
    """Fallback: Fetch flight arrivals from aeropuertomadrid-barajas.com (third-party)."""
    terminal_arrivals = {t: [] for t in TERMINALS}
    now = datetime.now(MADRID_TZ)
    ...
    logger.info(f"Fallback: Fetching flight data for time ranges: {time_ranges}")
                    ...
                    if response.status == 200:
                        ...
                        for record in flight_records:
                            try:
                                ...
                                if not terminal_div:
                                    continue
                                terminal = terminal_div.get_text(strip=True).replace("T4-S", "T4S")
                                if terminal not in terminal_arrivals:
                                    continue
                                ...
                                if status_div:
                                    ...
                                    if time_status_div:
                                        for cls in ['flightDetailDelayed', 'flightDetailAdvanced']:
                                            link = time_status_div.find('a', class_=cls)
                                            if link and not real_time:
                                                m = re.match(r'(\d{1,2}:\d{2})', link.get_text(strip=True))
                                                if m:
                                                    real_time = m.group(1)
                                    if "cancelado" in status_text:
                                        continue
                                    elif "llegado" in status_text:
                                        landed_time_str = real_time if real_time else arrival_time
                                        try:
                                            lp = landed_time_str.split(":")
                                            l_mins = int(lp[0]) * 60 + int(lp[1])
                                            n_mins = now.hour * 60 + now.minute
                                            diff = n_mins - l_mins
                                            if diff < 0:
                                                diff += 1440
                                            if diff <= 20:
                                ...
                                if not existing:
                                    terminal_arrivals[terminal].append({
                                        "time": final_time, "scheduled_time": arrival_time,
                                        "origin": origin, "flight_number": flight_number,
                                        "airline": airline, "terminal": terminal, "gate": "-",
                                        "status": status, "delay_minutes": delay_minutes,
                                    })
                            except Exception as e:
                                continue
        except Exception as e:
            logger.error(f"Fallback: Error fetching data for range {time_range}: {e}")
    
    for terminal in TERMINALS:
        terminal_arrivals[terminal].sort(key=lambda x: x['time'])
    total = sum(len(v) for v in terminal_arrivals.values())
    logger.info(f"Fallback: Flights fetched: {total} total")
    return terminal_arrivals
...
def filter_future_arrivals(arrivals: List[Dict], arrival_type: str = "flight") -> List[Dict]:
    """Filter arrivals to only include relevant ones.
    For flights: includes future flights AND recently landed ones (En tierra, Entregando equipaje, Finalizado).
    For trains: excludes arrived and cancelled.
    """
    now = datetime.now(MADRID_TZ)
    filtered = []
    ...
    if arrival_type == "flight":
        exclude_statuses = ["cancelado", "desviado", "pasado"]
    else:  # train
        exclude_statuses = ["llegado", "cancelado", "suprimido"]
    
    for arrival in arrivals:
        try:
            # Skip arrivals that are cancelled
            status = arrival.get("status", "").lower()
            if any(excl in status for excl in exclude_statuses):
                continue
            
            # Landed flights with valid statuses pass through directly
            if status in ["en tierra", "entregando equipaje", "finalizado"]:
...
async def get_flight_comparison(
    ...
    if is_past_window and time_start and time_end:
        ...
        # Merge recently finalized flights (detected from tracking - AENA drops them after FNL)
        # Only do this in real-time mode (no custom window)
        if not custom_time_window:
            try:
                finalized_recent = await get_recently_finalized_flights(within_minutes=60)
                if finalized_recent:
                    logger.info(f"[Flights] Found {len(finalized_recent)} recently finalized flights from tracking")
                    for f in finalized_recent:
                        term = f.get("terminal", "")
                        if term in all_arrivals:
                            # Avoid duplicates if AENA still returns the flight
                            existing_keys = {
                                (a.get("flight_number", ""), a.get("scheduled_time", a.get("time", "")))
                                for a in all_arrivals[term]
                            }
                            k = (f.get("flight_number", ""), f.get("scheduled_time", f.get("time", "")))
                            if k not in existing_keys:
                                all_arrivals[term].append(f)
            except Exception as e:
                logger.error(f"[Flights] Error merging finalized flights: {e}")
    ...
    for terminal in TERMINALS:
        ...
        # Count arrivals by category based on status
        upcoming_statuses = {"En tierra", "En vuelo", "En hora", "Retrasado", "Adelantado", "Estimado", "Programado"}
        delivering_statuses = {"Entregando equipaje"}
        past_statuses = {"Finalizado"}
        
        # Count large/wide-body aircraft in the relevant time windows
        large_30 = 0
        large_60 = 0
        
        if custom_time_window:
            upcoming_count = sum(1 for a in arrivals if a.get("status", "") in upcoming_statuses)
            delivering_count = sum(1 for a in arrivals if a.get("status", "") in delivering_statuses)
            past_count = sum(1 for a in arrivals if a.get("status", "") in past_statuses)
            count_30 = upcoming_count
            count_60 = upcoming_count
            delivering_30 = delivering_count
            delivering_60 = delivering_count
            past_30 = past_count
            past_60 = past_count
            # Large aircraft in the window (any non-cancelled flight that's wide-body)
            large_30 = sum(1 for a in arrivals if a.get("is_large"))
            large_60 = large_30
            # Lookahead not meaningful for custom historical windows
            next_after_30 = 0
            next_after_60 = 0
        else:
            # For real-time mode, count by status from the full list
            all_relevant = filter_future_arrivals(raw_arrivals, "flight")
            upcoming_30 = 0
            upcoming_60 = 0
            delivering_30 = 0
            delivering_60 = 0
            past_30 = 0
            past_60 = 0
            # Next half hour AFTER each window (early-warning lookahead)
            next_after_30 = 0  # arrivals in [+30, +60]
            next_after_60 = 0  # arrivals in [+60, +90]
            
            now_local = datetime.now(MADRID_TZ)
            for a in all_relevant:
                status = a.get("status", "")
                
                # Calculate time difference for all flights
                try:
                    t_parts = a["time"].split(":")
                    a_time = MADRID_TZ.localize(datetime(
                        now_local.year, now_local.month, now_local.day,
                        int(t_parts[0]), int(t_parts[1])
                    ))
                    if a_time < now_local - timedelta(hours=6):
                        a_time += timedelta(days=1)
                    elif a_time > now_local + timedelta(hours=12):
                        # Edge case: arrival is showing as future but actually was yesterday
                ...
                # Computed BEFORE the if/elif below so 'continue' statements don't skip it.
                if a.get("is_large"):
                    if status in upcoming_statuses:
                        ...
                        if -60 <= diff_mins <= 60:
                            large_60 += 1
                    elif status in delivering_statuses or status in past_statuses:
                        # Currently delivering or recently finalized wide-body
                        large_30 += 1
                        large_60 += 1
                
                if status in delivering_statuses:
                    # Count ALL flights currently in 'Entregando equipaje' status
                    # (real-time snapshot - they're delivering bags right now)
                    delivering_30 += 1
                    delivering_60 += 1
                elif status in past_statuses:
                    # Use finalized_at timestamp (when flight disappeared from feed)
                    # past_30: finalized in last 15 min (half of 30)
                    # past_60: finalized in last 30 min (half of 60)
                    finalized_at = a.get("finalized_at") or a.get("_finalized_at")
                    if finalized_at:
                        try:
                            ...
                            if finalized_at.tzinfo is None:
                                finalized_at = pytz.utc.localize(finalized_at)
                            mins_since_final = (now_local - finalized_at).total_seconds() / 60
                            if 0 <= mins_since_final <= 15:
                                past_30 += 1
                            if 0 <= mins_since_final <= 30:
                                past_60 += 1
                            continue
                        except Exception:
                            pass
                    # Fallback: use scheduled time diff
                    if -15 <= diff_mins <= 0:
                        past_30 += 1
                    if -30 <= diff_mins <= 0:
                        past_60 += 1
                elif status in upcoming_statuses:
                    # Count upcoming flights arriving within the time window
                    if -60 <= diff_mins <= 30:
                        upcoming_30 += 1
                    if -60 <= diff_mins <= 60:
                        upcoming_60 += 1
                    # Next half hour AFTER the window (lookahead)
                    if 30 < diff_mins <= 60:
                        next_after_30 += 1
                    if 60 < diff_mins <= 90:
                        next_after_60 += 1
            
            count_30 = upcoming_30
            count_60 = upcoming_60
        ...
        # Compute saturation predictions (demand 0-100)
        sat_30, sat_level_30 = calculate_saturation(count_30, delivering_30, next_after_30, large_30, past_30)
        sat_60, sat_level_60 = calculate_saturation(count_60, delivering_60, next_after_60, large_60, past_60)

        # Compute instant demand (last 30 min, status-driven)
        instant = calculate_instant_demand(arrivals)

        terminal_data[terminal] = TerminalData(
            terminal=terminal,
            arrivals=[FlightArrival(**a) for a in arrivals],
            total_next_30min=count_30,
            total_next_60min=count_60,
            delivering_30min=delivering_30,
            delivering_60min=delivering_60,
            score_30min=weighted_30,
            score_60min=weighted_60,
            past_30min=past_30,
            past_60min=past_60,
            next_after_30min=next_after_30,
            next_after_60min=next_after_60,
            large_30min=large_30,
            large_60min=large_60,
            saturation_30min=sat_30,
            saturation_60min=sat_60,
            saturation_level_30min=sat_level_30,
            saturation_level_60min=sat_level_60,
            instant_demand_pct=instant["pct"],
            instant_demand_level=instant["level"],
            instant_demand_points=instant["points"],
        )
...
async def upsert_tracked_flights(arrivals_by_terminal: Dict[str, List[Dict]]):
    """Upsert each currently-visible flight in the tracking collection.
    
    AENA's API drops flights once they reach 'Finalizado', so we track each flight's
    last known status and last_seen_at to detect when a flight in 'En tierra' or
    'Entregando equipaje' disappears from the feed (= it has finalized).
    """
    try:
        now = datetime.now(MADRID_TZ)
        # Use Madrid local date
        today = now.strftime("%Y-%m-%d")
        for terminal, arrivals in arrivals_by_terminal.items():
            for a in arrivals:
                fn = a.get("flight_number", "")
                t = a.get("scheduled_time") or a.get("time", "")
                if not fn or not t:
                    continue
                key = {
                    "flight_number": fn,
                ...
                try:
                    await flights_tracked_collection.update_one(key, update, upsert=True)
                except Exception as e:
                    logger.debug(f"[FlightTrack] upsert error for {fn}: {e}")
    except Exception as e:
        logger.error(f"[FlightTrack] Error in upsert_tracked_flights: {e}")


async def get_recently_finalized_flights(within_minutes: int = 60) -> List[Dict]:
    """Return flights that recently transitioned to 'Finalizado'.
    
    A flight is considered finalized when:
    - Its last seen status was 'En tierra' or 'Entregando equipaje'
    - It has not been seen in the AENA feed for at least 90 seconds
    - That disappearance happened within the last `within_minutes` minutes
    
    Returns flight dicts (same shape as fetch_aena_arrivals output) with status='Finalizado'.
    """
    try:
        now = datetime.now(MADRID_TZ)
        disappear_cutoff = now - timedelta(seconds=90)  # gone for at least 90 seconds
        window_cutoff = now - timedelta(minutes=within_minutes)
        
        cursor = flights_tracked_collection.find({
            "last_status": {"$in": ["En tierra", "Entregando equipaje"]},
            "last_seen_at": {"$gte": window_cutoff, "$lt": disappear_cutoff},
        })
        
        result = []
        async for doc in cursor:
            result.append({
                "time": doc.get("time", doc.get("scheduled_time", "")),
                ...
                "is_large": is_large_aircraft(doc.get("aircraft")),
                "finalized_at": doc.get("last_seen_at"),
            })
        return result
    except Exception as e:
        logger.error(f"[FlightTrack] Error in get_recently_finalized_flights: {e}")
        return []
...
async def get_public_summary():
    ...
    for terminal_name in ["T1", "T2", "T3", "T4", "T4S"]:
        ...
        # Count relevant flights: landed + future within 60min
        future_flights = []
        count_60min = 0
        landed_statuses = ["en tierra", "entregando equipaje", "finalizado"]
        ...
        for flight in arrivals:
            ...
            # Landed flights always count - passengers are at the airport NOW
            if flight_status in landed_statuses:
                count_60min += 1
                if len(future_flights) < 5:
...
async def get_street_work_data(
    ...
    for terminal in TERMINALS:
        ...
        if custom_time_window:
            ...
            # Also count landed flights as contributing to current demand
            landed_statuses = ["en tierra", "entregando equipaje", "finalizado"]
            landed_count = sum(1 for f in terminal_flights if f.get("status", "").lower() in landed_statuses)
            future_count += landed_count
    ...
    # Find hottest terminal - use flight arrivals data (same logic as flights tab)
    ...
    # Determine hottest terminal zone based on flight arrivals (most reliable data)
    best_terminal_zone = None
    best_zone_arrivals = 0
    for zone_name in ["T1", "T2-T3", "T4-T4S"]:
        zone_data = flight_arrivals_data.get(zone_name, {})
        zone_total = zone_data.get("future", 0) + zone_data.get("prev", 0)
        if zone_total > best_zone_arrivals:
            best_zone_arrivals = zone_total
            best_terminal_zone = zone_name
    
    if best_terminal_zone and terminal_scores.get(best_terminal_zone):
        best_terminal = terminal_scores[best_terminal_zone]
        hottest_terminal = best_terminal_zone
        ...
        terminal_taxi_query = {
            ...
            "location_name": {"$in": [hottest_terminal, hottest_terminal.replace("-", ""), *hottest_terminal.split("-")]},
            "reported_at": {"$gte": taxi_time_limit}
        }
        taxi_doc = await taxi_status_collection.find_one(
            terminal_taxi_query,
...
async def startup_db_client():
    ...
    try:
        ...
        # Create TTL index for flights_tracked (4 hours retention - we only care about recently seen)
        await flights_tracked_collection.create_index(
            "last_seen_at",
            expireAfterSeconds=14400,  # 4 hours
            background=True
        )
        logger.info("Created TTL index for flights_tracked (4 hour retention)")
        
        # Compound index for fast lookup
        await flights_tracked_collection.create_index(
            [("flight_number", 1), ("scheduled_time", 1), ("terminal", 1), ("arrival_date", 1)],
            background=True
        )
        logger.info("Created compound index for flights_tracked")
    except Exception as e:
    ...
    # Start daily events summary scheduler (generates at 5 AM)
    # NOTE: production used routers.events.daily_summary_scheduler. Replaced by
    # the standalone daily_summary router which regenerates on the first GET of
    # the day automatically (see routers/daily_summary.py).
    # from routers.events import daily_summary_scheduler
    # asyncio.create_task(daily_summary_scheduler())
    logger.info("Daily events summary handled by routers/daily_summary.py (lazy generation)")
